...
describe('getMessagesSince', () => {
  ...
  it('returns last N messages when no bot reply and no cursor exist', async () => {
    ...
    const msgs = await getMessagesSince('fresh@g.us', '', 'Andy', 10);
    expect(msgs).toHaveLength(10);
...
describe('storeChatMetadata', () => {
  ...
  // Note: timestamp MAX behavior differs slightly with Supabase upsert
  // (Supabase upsert replaces the value; preserving MAX requires an RPC or raw SQL).
  // For now the last-written timestamp wins — acceptable for the runtime.
});
